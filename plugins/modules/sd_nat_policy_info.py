...
def main():
    ...
    if module.params["id"]:
        nat_policies = mgr.get_by_id(module.params["id"])
        module.exit_json(nat_policies=nat_policies, changed=False)
    else:
        if module.params["name"] or module.params["ip_address"]:
            # call get_address for greater detail and single element list
            nat_policies = mgr.get_all(
                name=module.params["name"],
                ip_address=module.params["ip_address"]
            )
        else:
            # call general get_nat_policies which will return full list only if no filters are provided
            nat_policies = mgr.get_all()
